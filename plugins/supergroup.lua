...
local function check_member_super(cb_extra, success, result) 
  ...
  if success == 0 then 
    send_large_msg(receiver, "نعتذر منك ولكن هذا فقط للمشرفين") 
  end 
...
local admin_num = "☑️ عدد المشرفين : "..result.admins_count.."\n" 
...
--vardump(result) 
local text = "🌐 قائمه الايدي الخاصبه باعضاء المجموعة 🆔"..cb_extra.receiver.."\n\n" 
local i = 1 
for k,v in pairsByKeys(result) do 
if not v.print_name then 
    name = " " 
...
local function lock_group_spam(msg, data, target) 
  ...
  if not is_owner(msg) then 
    return " لا يمكنك التحكم ⚠️ في البوت للمشرفين فقط" 
  end 
...
local function set_public_membermod(msg, data, target) 
  if not is_momod(msg) then 
    return "❌ ⚠️ لا يمكنك التحكم في البوت للمشرفين " 
  end 
...
local function promote_admin(receiver, member_username, user_id) 
  ...
  if data[group]['moderators'][tostring(user_id)] then 
    return send_large_msg(receiver, member_username..'📌 هوه بالفعل ☑️ ضمن المشرفين ') 
  end 
...
local function promote2(receiver, member_username, user_id) 
  ...
  data[group]['moderators'][tostring(user_id)] = member_tag_username 
  save_data(_config.moderation.data, data) 
  send_large_msg(receiver, member_username..' 📌 تم رفعك مشرف في المجموعة') 
end 
...
local function demote2(receiver, member_username, user_id) 
  ...
  if not data[group]['moderators'][tostring(user_id)] then 
     return send_large_msg(receiver, member_username..' 📌 تم بالفعل ☑️ تنزيله من الاشراف') 
  end 
  data[group]['moderators'][tostring(user_id)] = nil 
  save_data(_config.moderation.data, data) 
  send_large_msg(receiver, member_username..' 📌 تم ☑️ تنزيلك من الاشراف على المجموعة') 
end 
...
local function modlist(msg) 
  ...
  if next(data[tostring(msg.to.id)]['moderators']) == nil then 
    return '📌 لا يوجد ⚠️ مشرف حاليٱ ❗️' 
  end 
  local i = 1 
  local message = '\n🌐 قائمه المشرفين ☑️️ ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n' 
...
function get_message_callback(extra, success, result) 
    ...
    elseif get_cmd == 'الايدي' and result.action then 
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then 
               return send_large_msg("channel#id"..channel_id, "📌  لا يمكنك ⚠️ طرد المشرف ❌ او المدير ") 
    end 
    ...
    elseif get_cmd == "رفع مشرف" then 
        ...
        if result.from.username then 
            text = "@"..result.from.username.."   📌 تم ☑️ رفعك ضمن المشرفين" 
        else 
            text = "[ "..user_id.." ]   📌 تم ☑️ رفعك ضمن المشرفين " 
        end 
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set: ["..user_id.."] as admin by reply") 
        send_large_msg(channel_id, text) 
    elseif get_cmd == "انزال مشرف" then 
        local user_id = result.from.peer_id 
        local channel_id = "channel#id"..result.to.peer_id 
        if is_admin2(result.from.peer_id) then 
            return send_large_msg(channel_id, "📌 ❌ لا يمكنك ⚠️ انزال مشرف") 
        end 
        channel_demote(channel_id, "user#id"..user_id, ok_cb, false) 
        if result.from.username then 
            text = "@"..result.from.username.."  📌 تم ☑️ ازالتك من الاشراف" 
        else 
            text = "[ "..user_id.." ]  📌 تم ☑️ ازالتك من الاشراف" 
        end 
    ...
    elseif get_cmd == "رفع المدير" then 
        ...
        local channel_id = 'channel#id'..result.to.peer_id 
            ...
            if result.from.username then 
                text = "@"..result.from.username.." [ "..result.from.peer_id.." ]   📌 تم ☑️ رفعك مدير  " 
            else 
                text = "[ "..result.from.peer_id.." ]   📌 تم ☑️ رفعك مدير  " 
            end 
            send_large_msg(channel_id, text) 
        end 
...
send_large_msg(receiver, "["..user_id.."] 📌 تم الغاء الكتم ") 
        elseif is_admin1(msg) then 
            mute_user(chat_id, user_id) 
            send_large_msg(receiver, " ["..user_id.."] 👌  تم تفعيل الكتم على") 
        end 
    end 
end 
...
local function cb_user_info(extra, success, result) 
    ...
    if get_cmd == "تنزيل اداري" then 
        if is_admin2(result.peer_id) then 
            return send_large_msg(receiver, "لا يمكنك لانك لست مشرف") 
        end 
        local user_id = "user#id"..result.peer_id 
        channel_demote(receiver, user_id, ok_cb, false) 
        if result.username then 
            text = "@"..result.username.."   📌 تم ازالته من الاشراف" 
            send_large_msg(receiver, text) 
        else 
            text = "[ "..result.peer_id.." ]   📌 تم ازالته من الاشراف" 
            send_large_msg(receiver, text) 
        end 
    elseif get_cmd == "رفع ادمن" then 
        if result.username then 
...
local function callbackres(extra, success, result) 
  ...
  elseif get_cmd == "invite" then 
    ...
    elseif get_cmd == "تنزيل اداري" then 
        ...
        local channel_id = extra.channel 
        if is_admin2(result.peer_id) then 
            return send_large_msg(receiver, " 📌 لا يمكنك تنزيله لانك لست مشرف") 
        end 
        channel_demote(channel_id, user_id, ok_cb, false) 
        if result.username then 
            text = "@"..result.username.." 📌 تم انزاله من الاشراف  " 
            send_large_msg(channel_id, text) 
        else 
            text = "@"..result.peer_id.."  📌 تم انزاله من الاشراف  " 
            send_large_msg(channel_id, text) 
        end 
        local receiver = extra.channel 
        local user_id = result.peer_id 
    ...
    if vusername == member or vpeer_id == memberid then 
     ...
     local sender = cb_extra.msg.from.id 
      ...
      if is_momod2(user_id, channel_id) and not is_admin2(sender) then 
        return send_large_msg("channel#id"..channel_id, "📌  لايمكنك طرد المدير او المشرف") 
      end 
      if is_admin2(user_id) then 
        return send_large_msg("channel#id"..channel_id, "📌  لا يمكنك طرد المشرف") 
      end 
      if v.username then 
        text = "" 
...
elseif get_cmd == "رفع اداري" then 
   for k,v in pairs(result) do 
    ...
    if vusername == member or vpeer_id == memberid then 
      ...
      channel_set_admin(channel_id, user_id, ok_cb, false) 
      if v.username then 
        text = "@"..v.username.." ["..v.peer_id.."]📌 لقد تم رفعك مشرف بنجاح " 
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set admin @"..v.username.." ["..v.peer_id.."]") 
      else 
        text = "["..v.peer_id.."]   📌 لقد تم رفعك مشرف بنجاح" 
    ...
    for k,v in pairs(result) do 
        ...
        if vusername == member or vpeer_id == memberid then 
            ...
            if group_owner then 
                ...
                if result.username then 
                    text = member_username.." ["..v.peer_id.."]  📌 تم رفعك مشرف  " 
                else 
                    text = "["..v.peer_id.."] 📌 تم رفعك مشرف  " 
                end 
            end 
...
return "📌 مدير المجموعة  📌["..group_owner..']' 
        ...
        if matches[1] == 'رفع ادمن' then 
          if not is_momod(msg) then 
            ...
            if not is_owner(msg) then 
                return "⚠️ خطاء المشرفين فقط يحق لهم فعل ذالك" 
            end 
            if type(msg.reply_id) ~= "nil" then 
                local cbreply_extra = { 
                    get_cmd = 'رفع ادمن', 
        ...
        if matches[1] == 'مسح' then 
            ...
            if matches[2] == 'الادمنيه' then 
                if next(data[tostring(msg.to.id)]['moderators']) == nil then 
                    return '📌 عذرا لا يوجد مشرفين ليتم مسحهم ' 
                end 
                for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do 
                ...
                return '📌 تم ☑️ مسح ❌ قائمه المشرفين بنجاح' 
            end 
