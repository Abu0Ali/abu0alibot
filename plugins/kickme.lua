...
local hash = 'kick:'..msg.to.id..':'..msg.from.id 
   ...
   send_large_msg(get_receiver(msg), "تم الغاء ازالتك من المجموعة 🌐") 
     redis:del(hash, true) 
   ...
   local hash = 'kick:'..msg.to.id..':'..msg.from.id 
    if redis:get(hash) then 
        if redis:get(hash) == "ok" then 
         ...
         return ' لقد تم ازالتك من المجموعة ('..msg.to.title..')⚠️' 
        end 
      end 
    end 
